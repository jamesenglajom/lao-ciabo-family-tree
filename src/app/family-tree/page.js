...
export async function generateMetadata() {
  ...
  return {
    title: { absolute: s.treeTitle },
    description: s.treeMetaDescription,
    alternates: { canonical: "/family-tree" },
  };
}

export default async function FamilyTreePage() {
  const [data, s] = await Promise.all([getFamilyChartData(), getSiteSettings()]);
    ...
    <Container className="flex flex-col gap-6 py-10">
      <div className="flex flex-col gap-2">
        ...
        <p className="max-w-2xl text-sm leading-relaxed text-ink-soft">
          {s.treeDescription} Drag to pan, scroll to zoom, and click any card to make that person
          the new center of the tree. Use the search field to jump straight to a name.
        </p>
      </div>

      <div className="family-chart-shell glass h-[70vh] min-h-130 w-full overflow-hidden rounded-3xl">
        <FamilyChartTree data={data} label={`${s.treeTitle} (interactive chart)`} />
      </div>
